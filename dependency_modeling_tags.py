...
def main():
    ...
    ## Other parameters
    parser.add_argument("--no_cuda", type=eval, default=False,
    ...
    parser.add_argument("--max_span_length", default=1, type=int,
    ...
    parser.add_argument("--tag", default="", type=str,
                        help="Name of the tag to mask out.")
    ...
    logger.info('Data: ', args.data_name, ' model_type: ', args.model_type, ' model_name_path: ', args.model_name_or_path,
    ...
    data = np.load(str(data_path / f"tags_{args.model_type}.npy"), allow_pickle=True).item()
    data = data[args.tag]
    ...
    original_texts = [example["original_text"] for example in data]
    tokenized_texts = [example["tokenized_text"] for example in data]
    tag_indices = [example["masked_index"] for example in data]

    original_texts = original_texts[:args.max_num_examples]
    tokenized_texts = tokenized_texts[:args.max_num_examples]
    tag_indices = tag_indices[:args.max_num_examples]
    ...
    for i in range(len(tokenized_texts)):
        ...
        span_start_index = tag_indices[i]
        ...
        for context_size in context_sizes:
            ...
            indexed_tokens = tokenizer.build_inputs_with_special_tokens(tokenizer.convert_tokens_to_ids(context_masked_text))
            # Define sentence
            ...
            if args.model_type in ['xlnet']:
                ...
                target_mapping = torch.zeros((1, args.span_length, tokens_tensor.shape[1]),
                                             dtype=torch.float)
